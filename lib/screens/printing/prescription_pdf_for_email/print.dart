...
class PdfEmail extends StatefulWidget {
  ...
  var generalSetting;
   ...
   PdfEmail({super.key, required this.selectedClinicalDataColumn1, required this.selectedMedicineDataColumn2, required this.printPatientAndAppointmentInfo,
     required this.advices, this.prescriptionSettingData, this.generalSetting});

  @override
  State<PdfEmail> createState() => _PdfEmailState();
}
...
class _PdfEmailState extends State<PdfEmail> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      //       canDebug: false,
      //       build: (format) => smsPdf(widget.selectedClinicalDataColumn1,widget.selectedMedicineDataColumn2,widget.printPatientAndAppointmentInfo,widget.advices,widget.prescriptionSettingData, widget.generalSetting)
      //     ),
      //   ),
      // )
    );
...
Future<File> smsPdf(dataColumn1,dataColumn2,patientAndAppointmentInfo,advices, prescriptionSettingData, generalSetting,patientHistory) async {
  final PrescriptionPrintPageSetupController printSetup = Get.put(PrescriptionPrintPageSetupController());
...
pw.Widget _buildHeader(bool isFirstPage, prescriptionSettingData, headerImageHeight, headerHeight,) {
  ...
  final PrescriptionPrintPageSetupController printSetup = Get.put(PrescriptionPrintPageSetupController());
  if(printSetup.headerFooterAndBgImage[0]['headerImage'].isNotEmpty){
    print(printSetup.headerFooterAndBgImage[0]['headerImage']);
  }else{
    print("empty");
  }
  ...
  return pw.Padding(
      ...
      child: pw.Column(
          children: [
            ...
            if(printSetup.headerFooterAndBgImage[0]['headerImage'].isEmpty)
              pw.Container(
                width: double.infinity,
                child: pw.SizedBox(
                  height: headerHeight ,
...
pw.Widget _buildFooter(prescriptionSettingData, footerImageHeight, footerHeight) {
  ...
  return pw.Padding(
      ...
      child: pw.Container(
          child: pw.Column(
              children: [
                if(printSetup.headerFooterAndBgImage[0]['footerImage'].isNotEmpty)
                  pw.Padding(
                    padding: pw.EdgeInsets.symmetric(horizontal: 0.0),
                    child: pw.Container(
                      width: double.infinity,
                ...
                if(printSetup.headerFooterAndBgImage[0]['footerImage'].isEmpty)
                  pw.Container(
                    width: double.infinity,
                    child: pw.SizedBox(
                      height: footerHeight ,
                                ...
                                children: [
                                  item.isNotEmpty ? pw.Column(
                                      children: [
                                        ...
                                        ) : clinicalDataListPw(item,clinicalDataMargin, customFont, fontSize, ),
...
pw.Widget _buildDataRowPatientInfo(patientAndAppointmentInfo, prescriptionSettingData, generalSetting) {
  ...
  if(patientInfoData.dob != null && patientInfoData.dob != "" ){
    ...
    if(generalSetting.any((element) => element['section'] == Settings.print && element['label'] == "AgeYearMonthDayMin")){
      ageString = "${ age.years.toString() }Y-${age.months.toString()}M-${ age.days.toString() }D-${age.hours.toString()}H-${age.minutes.toString()}Min";
    }else if(generalSetting.any((element) => element['section'] == Settings.print && element['label'] == "AgeYearMonthDay")){
      ageString = "${ age.years.toString() }Y-${ age.months.toString() }M- ${ age.days.toString() }D";
...
pw.Widget clinicalDataListPw(item,clinicalDataMargin,customFront, fontSize){
  List<pw.TextSpan> textSpans = [];
