...
class PrintPreview extends StatefulWidget {
  ...
  final printPatientAndAppointmentInfo;
  final List advices;
  ...
  var patientHistory;
   ...
   PrintPreview({super.key, required this.selectedClinicalDataColumn1, required this.selectedMedicineDataColumn2, required this.printPatientAndAppointmentInfo,
...
class _PrintPreviewState extends State<PrintPreview> {
  ...
  Widget build(BuildContext context) {
    ...
    if(!isFollowPrint){
      clinicalData = widget.selectedClinicalDataColumn1;
    }else{
      clinicalData = [];
    }
      ...
      appBar: AppBar(
          ...
          title: ListTile(
            title: Wrap(
              ...
              children: [
                Text("Prescription Print Preview", style: TextStyle(color: Colors.white, fontSize: screenWidth > 600 ? 20 : 16),),
                Wrap(
                  alignment: WrapAlignment.spaceBetween,
                  crossAxisAlignment: WrapCrossAlignment.center,
                  children: [
                    Text("Follow Up Print", style: TextStyle(color: Colors.white, fontSize: screenWidth > 600 ? 20 : 16),),
                    Switch(
                      value: isFollowPrint,
                      onChanged: (value) {
                        setState(() {
      ...
      body: Center(
        child: Container(
          ...
          child: PdfPreview(
            ...
            canDebug: false,
            build: (format) => _printPdf(clinicalData,widget.selectedMedicineDataColumn2,widget.printPatientAndAppointmentInfo,widget.advices,widget.prescriptionSettingData, widget.generalSetting, widget.patientHistory, isFollowPrint),
          ),
        ),
      )
    );
...
Future<Uint8List> _printPdf(dataColumn1,dataColumn2,patientAndAppointmentInfo,advices, prescriptionSettingData, generalSetting, patientHistory, isFollowPrint) async {
                ...
                children: [
                  ...
                  pw.Column(
                    children: [
                      ...
                      i == 0 ? _buildDataRowPatientInfo(patientAndAppointmentInfo,prescriptionSettingData, generalSetting,isFollowPrint) : pw.SizedBox(height: 5),
                  ...
                  pw.Expanded(
                    child: _buildContent(column1Sublist, column2Sublist, prescriptionSettingData, patientAndAppointmentInfo, advices, totalPages, i + 1 , customFont, sidebarWidth,fontSize, itemsPerPageColumn2, itemsPerPageColumn1, totalItemsColumn1, totalItemsColumn2,generalSetting,qrCodeData,patientHistory,clinicalDataListIcon,isFollowPrint),
                  ),
...
pw.Widget _buildContent(List<String> column1Data, List column2Data, prescriptionSettingData, patientAndAppointmentInfo,List advices, totalPages, i, customFont,sidebarWidth,fontSize, itemsPerPageColumn2, itemsPerPageColumn1, totalPagesColumn1, totalPagesColumn2, generalSetting,qrCodeData,patientHistory,clinicalDataListIcon,isFollowPrint) {
  ...
  double clinicalLeftDataMargin = double.parse(printSetup.marginLeftClinicalData.text.toString()) * 72;
  double clinicalRightDataMargin = double.parse(printSetup.marginRightBrandData.text.toString()) * 72;
  double brandRightDataMargin = double.parse(printSetup.marginRightBrandData.text.toString()) * 72;
  double brandLeftDataMargin = double.parse(printSetup.marginLeftBrandData.text.toString()) * 72;
...
// Iterate through the data list and process each 'name'
   ...
   return pw.Padding(
    ...
    child: pw.Row(
        ...
        children: [
          pw.Padding(
            padding: pw.EdgeInsets.only(top: clinicalDataStartingTopMargin, bottom: 0.0, left: clinicalLeftDataMargin, right: clinicalRightDataMargin),
            child: pw.SizedBox(
              width: sidebarWidth,
              child: pw.Expanded(
                child: pw.Column(
          ...
          pw.Expanded(
              child:  pw.Padding(
                padding:  pw.EdgeInsets.only(top: rxDataStartingTopMargin, bottom: 0.0, left: brandLeftDataMargin, right: brandRightDataMargin),
                child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
                    children: <pw.Widget>[
                      if(!isFollowPrint)
...
pw.Widget _buildDataRowPatientInfo(patientAndAppointmentInfo, prescriptionSettingData, generalSetting, isFollowPrint) {
  ...
  if(patientInfoData.dob != null && patientInfoData.dob != "" ){
    print(patientInfoData.dob);
    var age;
    try{
      DateTime dob = parseDate(patientInfoData.dob.toString());
